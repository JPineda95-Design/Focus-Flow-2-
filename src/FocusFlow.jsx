...
import {
  ...
  Battery, Lightbulb, Target, Rocket, BookOpen, CalendarCheck, Repeat
} from "lucide-react";
...
function SessionConfigOverlay({ onClose, onSave, initialDurations }) {
...
════════════════════════════════════════════ */
const INIT_TASKS = [
  { id:1, label:"Revisar PRs del sprint actual",   duration:"~30 min", done:false, priority:"high" },
  { id:2, label:"Actualizar documentación de API",  duration:"~45 min", done:false, priority:"high" },
  { id:3, label:"Reunión de sincronización",        duration:"30 min",  done:true,  priority:"med"  },
  { id:4, label:"Refactorizar módulo de pagos",     duration:"~1h",     done:false, priority:"low"  },
];
...
function PanelScreen({ onNavigate, frogTask }) {
  ...
  return (
    <div className={`page${running?" session-active":""}${running&&mode.id==="deep-flow"?" deep-flow-active":""}`}>
      ...
      {/* Modals */}
      {cfgOpen       && <SessionConfigOverlay onClose={()=>setCfgOpen(false)} onSave={handleConfigSave} initialDurations={customDurations}/>}
      ...
      <div className="container">
        ...
        <div style={{ display:"grid", gridTemplateColumns:"1fr 380px", gap:20, alignItems:"stretch" }}>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
            ...
            {/* ── TAREAS DE SESIÓN ── */}
            <div className="m3-card fi fi2 deep-flow-blur" style={{ padding:24, flex:1 }}>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:14 }}>
              ...
              {tasks.length === 0 ? (
                ...
                <div style={{ display:"flex", flexDirection:"column", gap:2 }} role="list" aria-label="Lista de tareas">
                  {tasks.map(t=>(
                    <div key={t.id}
                      ...
                      <div style={{ width:8, height:8, borderRadius:"50%", background:dot[t.priority], flexShrink:0 }}/>
                    </div>
                  ))}
                </div>
              )}
          ...
          <div className="fi" style={{ display:"flex", flexDirection:"column" }}>
            <div className="m3-card" style={{
              ...
              {/* Selector de modo */}
              <div>
                <div className="label-md" style={{ color:"var(--md-on-surface-variant)", marginBottom:10 }}>MODO DE SESIÓN</div>
                <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
                  {SESSION_MODES.map(m=>(
...
function InicioScreen({ frogTask, onNavigate }) {
  const { userPlan, openPlans } = usePlan();
  const [deepFlowSlots, setDeepFlowSlots] = useState(2);
    ...
    <div className="page">
      <div className="container">
        ...
        <div className="fi fi3" style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:16, marginBottom:16, alignItems:"stretch" }}>
          ...
          {/* COL A: Bloques de Concentración Profunda */}
          <div className="m3-card" style={{ padding:20, display:"flex", flexDirection:"column" }}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:8 }}>
              <div style={{ display:"flex", alignItems:"center", gap:6 }}>
                <Target size={14} style={{ color:"var(--secondary)", flexShrink:0 }}/>
                <span className="title-sm" style={{ fontSize:".82rem" }}>Bloques de Concentración Profunda</span>
              </div>
            </div>
            <p className="body-sm" style={{ color:"var(--md-on-surface-variant)", marginBottom:16, lineHeight:1.6 }}>
              Tu energía cognitiva de alta calidad es <strong>limitada y valiosa</strong>. Designa estos bloques para lo que más importa.
            </p>
            <span style={{ fontSize:".7rem", fontWeight:700, color:"var(--accent)",
              background:"var(--md-secondary-container)", padding:"3px 10px", borderRadius:99,
              alignSelf:"flex-start", marginBottom:14 }}>
              {deepFlowSlots} restante{deepFlowSlots!==1?"s":""} hoy
            </span>
            {/* Slots verticales como en el bosquejo */}
            <div style={{ display:"flex", flexDirection:"column", gap:10, flex:1, justifyContent:"flex-end" }}>
              {[1,2,3].map(slot => {
                const available = slot <= deepFlowSlots;
                return (
                  <button key={slot}
                    onClick={()=>{ if(available) setDeepFlowSlots(s=>s-1); }}
                    style={{ width:"100%", padding:"12px 16px", borderRadius:"var(--rad-md)",
                      border:`1px solid ${available?"var(--secondary)":"var(--md-outline-variant)"}`,
                      background:available?"rgba(26,35,126,.06)":"var(--md-surface)",
                      cursor:available?"pointer":"default", transition:"all .2s",
                      fontFamily:"var(--font-display)", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:4 }}>
                    <span style={{ fontSize:"1.2rem", lineHeight:1 }}>{available?"⚡":"✓"}</span>
                    <span style={{ fontSize:".78rem", fontWeight:700,
                      color:available?"var(--secondary)":"var(--md-outline)" }}>
                      {available?"Disponible":"Usado"}
                    </span>
                  </button>
                );
              })}
          ...
          <div className="m3-card" style={{ padding:20, display:"flex", flexDirection:"column", alignItems:"center" }}>
            ...
            <div style={{ position:"relative", display:"flex", alignItems:"center", justifyContent:"center", flex:1, margin:"8px 0" }}>
              <DonutChart pct={pct} size={210} stroke={18}/>
        ...
        <div className="fi" style={{ marginBottom:16 }}>
          <div className="m3-card fi" style={{ padding:"13px 20px", display:"flex", alignItems:"center", gap:12 }}>
...
/* ── initial events ── */
const INIT_EVENTS=[
  { id:1, day:0, startH:9,  startM:0,  endH:11, endM:0,  title:"Deep Flow · Código",   cat:0 },
  { id:2, day:1, startH:9,  startM:0,  endH:10, endM:30, title:"Diseño wireframes",     cat:1 },
  { id:3, day:1, startH:11, startM:0,  endH:13, endM:0,  title:"Diseño UX",             cat:1 },
  { id:4, day:2, startH:9,  startM:0,  endH:11, endM:0,  title:"Pruebas de usabilidad", cat:0 },
  { id:5, day:3, startH:10, startM:0,  endH:13, endM:0,  title:"Desarrollo frontend",   cat:1 },
  { id:6, day:2, startH:13, startM:0,  endH:15, endM:30, title:"Diseño de app",         cat:0 },
  { id:7, day:4, startH:9,  startM:0,  endH:11, endM:0,  title:"Vital Reset",           cat:2 },
];
      ...
      {detailEv && (
        <div className="scrim" onClick={()=>setDetailEv(null)}>
          <div className="m3-dialog" style={{ maxWidth:320, width:"90%", padding:20 }}
            ...
            <button className="btn-filled" style={{ width:"100%", justifyContent:"center", marginTop:16, background:"var(--md-error)" }}
      ...
      <SessionStatusContext.Provider value={sessionCtxValue}>
        ...
        <main role="main">
          ...
          <ScreenSlot active={screen==="panel"}>
            <PanelScreen onNavigate={handleNavClick} frogTask={frogTask}/>
          </ScreenSlot>
